...
team_red_scores <- function(matches,team_key) {
  idx <- team_red_alliances(matches,team_key)
  data.frame(time = matches$actual_time[idx], 
             score = matches$alliances.red.score[idx],
             opp_score = matches$alliances.blue.score[idx])
}

team_blue_scores <- function(matches,team_key) {
  idx <- team_blue_alliances(matches,team_key)
  data.frame(time = matches$actual_time[idx], 
             score = matches$alliances.blue.score[idx], 
             opp_score = matches$alliances.red.score[idx])
}

team_scores <- function(matches,team_key) {
  scores <- rbind(team_red_scores(matches,team_key),team_blue_scores(matches,team_key))
  scores[order(scores$time),]
}

perf <- team_scores(matches,our_team)
plot(perf$time,perf$score,type="p",col="black",ylim=c(0,500))
lines(perf$time,perf$opp_score,type="p",col="red")
